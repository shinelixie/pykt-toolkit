...
class MOCKTDataset(Dataset):
    ...
    def __init__(self, file_path, input_type, folds, qtest=False):
        super(MOCKTDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        for key in self.dori:
            if key in ["masks", "smasks", "ground_seq"]:
                continue
        ...
        dcur["ground_seq"] = self.dori["ground_seq"][index]
    ...
    def __load_data__(self, sequence_path, folds, pad_val=-1):
        ...
        """
        dori = {"qseqs": [], "cseqs": [], "rseqs": [], "tseqs": [], "utseqs": [], "smasks": [], "ground_seq": []}
        ...
        for i, row in df.iterrows():
            ...
            count_non_negative = sum(1 for x in responses if x != -1)
            dori["ground_seq"].append([count_non_negative])
        ...
        for key in dori:
            if key not in ["rseqs","ground_seq"]:#in ["smasks", "tseqs"]:
